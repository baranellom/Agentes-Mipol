...
while ($art_magento = mysqli_fetch_array($Articulos_magento)):
	...
	$Consulta_Sustitutos = "SELECT GROUP_CONCAT(r_prd_prd.prd_id2) as sustitutos from r_prd_prd WHERE r_prd_prd.prd_id1 = ".$art_magento['id_gestuc']." group by r_prd_prd.prd_id1 order by prd_id2 ;";
	...
	if ( ! is_null($Art_Sustitutos['sustitutos']))
	{
		...
		$Modificar_registro = "UPDATE articulos_magento SET related_skus='".$Art_Sustitutos['sustitutos']."' WHERE  sku='".$art_magento['sku']."' LIMIT 1;";
