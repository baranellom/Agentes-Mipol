...
while ($art_magento = mysqli_fetch_array($Articulos_magento)):
	...
	$Consulta_Sustitutos = "SELECT GROUP_CONCAT( CONCAT(r_prd_prd.prd_id2,'-1') ) as sustitutos from r_prd_prd WHERE r_prd_prd.prd_id1 = ".$art_magento['id_gestuc']." AND r_prd_prd.prd_id2 IN (SELECT DISTINCT articulos_magento.id_gestuc FROM articulos_magento WHERE product_online = 1) group by r_prd_prd.prd_id1 order by prd_id2 ;";
