...
if ($en_ejecucion = mysqli_fetch_array($Chequeo_ejecucion)){
    ...
    #-- Cargo ejecucion 
    if (mysqli_query ( $enlace, "INSERT INTO actualizacion_ecommerce (id, tienda_id, tipo, fechahora_inicio) VALUES ('".$id_max["id"]."', '1', '2', now());" )){
        printf ( "Se inició una nueva actualización de Precios\r\n" );
...
$fecha_inicio = mysqli_query ( $enlace, "SELECT fechahora_inicio FROM actualizacion_ecommerce WHERE tipo = 2 AND fechahora_fin IS NOT NULL ORDER BY id DESC LIMIT 1;" );
...
$Consulta_precios = $Consulta_precios . "'". $fecha_inicial["fechahora_inicio"]."');";
...
printf ( "Se actualizarán ".$Cant_articulos_actualizados." Precios\r\n" );
...
while ($art_magento = mysqli_fetch_array($Articulos_magento)):
    ...
    //print_r($response);
    echo "\r\nPrecio del Articulo ".$art_magento["sku"]." modificado. - " . date('d/m/Y H:i:s') . "\r\n";
